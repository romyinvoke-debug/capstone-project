...
// Tabel 'activity_log' untuk mencatat riwayat aktivitas pengguna
export const activityLog = pgTable('activity_log', {
    id: serial('id').primaryKey(),
    userId: integer('user_id').references(() => users.id),
    userName: varchar('user_name', { length: 100 }),
    action: varchar('action', { length: 20 }), // 'CREATE' atau 'DELETE'
    targetTable: varchar('target_table', { length: 50 }), // 'data_uttp'
    targetId: integer('target_id'),
    targetSummary: text('target_summary'), // JSON format dari ringkasan data yang diubah
